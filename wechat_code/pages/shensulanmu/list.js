...
this.setData({
addAuth: utils.isAuth("shensulanmu", "新增") || utils.isAuthFront("shensulanmu", "新增"),
delAuth: utils.isAuth("shensulanmu", "删除") || utils.isAuthFront("shensulanmu", "删除"),
editAuth: utils.isAuth("shensulanmu", "修改") || utils.isAuthFront("shensulanmu", "修改"),
})
if(options?.userid) {
this.setData({
...
page: this.data.currPage,
    limit: this.data.pageSize
            ...
            if(this.data.shenshudianhua){
            searchForm['shenshudianhua'] = '%' + this.data.shenshudianhua + '%'
        }
    

let res = {};
if(this.data.userid) {
res = await page(`shensulanmu`, searchForm);
} else {
res = await list(`shensulanmu`, searchForm);
}
...
wx.navigateTo({
url: `/pages/shensulanmu/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("shensulanmu",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
