...
goodid: this.data.detailList.id,
    goodname:  this.data.detailList.shenshudianhua,
...
const {
data
} = await detail("shensulanmu",id)
this.setData({
payAuth:utils.isAuthFront('shensulanmu','支付')
})
...
const resUpdate = await update('shensulanmu', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','shensulanmu');
...
async reply() {
const detailList = this.data.detailList
const res = await update("shensulanmu", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
